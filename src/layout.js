...
export function renderNewListItem(item) {
  ...
  let matchedPriority = options.find(
    (option) => option.label === item.priority
  );

  let taskPriority = document.createElement("div");
  taskPriority.classList.add("priority-symbol");
  taskPriority.textContent = matchedPriority
    ? matchedPriority.symbol
    : item.priority;
...
export function addNewTask() {
  ...
  newTask.addEventListener("click", () => {
    ...
    taskSubmitButton.addEventListener("click", (e) => {
      ...
      let makeTask = new toDoItem(
        taskInputTitle.value,
        taskInputDescr.value,
        taskInputDue.value,
        taskInputPriority.value,
        taskInputAssignList.value
      );
      const list = masterList.find(
        (choice) => choice.listName === taskInputAssignList.value
      );
